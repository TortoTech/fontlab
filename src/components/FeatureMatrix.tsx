...
interface Props {
  ...
  fontTick: number
  loadGoogle: (family: string) => Promise<boolean>
...
export default function FeatureMatrix({ customFonts, fontTick, loadGoogle, onUseFont, onToast }: Props) {
  ...
  }, [results, query, source, showUnavailable])
  ...
  const handleLoad = async (family: string) => {
    ...
    const ok = await loadGoogle(family)
    setLoading((s) => {
      const n = new Set(s)
      n.delete(family)
      return n
  ...
  return (
    <div>
      ...
      <div className="mb-3 flex flex-wrap items-center gap-x-4 gap-y-1 rounded-lg border border-zinc-200 bg-white px-3 py-2 text-xs text-zinc-500 dark:border-zinc-800 dark:bg-zinc-900 dark:text-zinc-400">
        ...
        <span className="ml-auto hidden text-zinc-400 sm:inline lg:hidden">
          粗体/斜体：网络字体按实际加载的字形判断；本地字体为渲染对比推断，无法区分真实字形与浏览器合成
        </span>
      </div>
      ...
      <p className="mt-3 text-xs text-zinc-400 dark:text-zinc-500">
        检测说明：等宽通过字符宽度测量判断；中文/拉丁/数字通过字形覆盖检测；粗体/斜体对已加载的网络字体读取真实字形，对本地字体采用渲染对比推断，浏览器合成的粗体/斜体会标记为「~」。
      </p>
    </div>
  )
}
