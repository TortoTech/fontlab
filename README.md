...
> 在线预览：<https://tortotech.github.io/lab/>
...
- 检测方式：Google Fonts 的 OpenType 特性（连字/表格数字/旧式数字/小型大写/分数/上下标/序数）在 CI 离线解析字体文件生成静态数据，运行时零检测；已加载字体按实际字形检测（等宽=字符宽度测量，粗体/斜体=读取真实字形或渲染对比推断）；未加载的 Google 字体按官方目录元数据推导，无需下载；本地字体在 Chrome/Edge 授权后经 Local Font Access API 精确检测（真实字重/斜体 + fontkit 解析字形表判断中文覆盖），不支持或未授权时自动回退到宽度对比启发式；自定义上传字体在上传时解析一次特性并随记录保存
